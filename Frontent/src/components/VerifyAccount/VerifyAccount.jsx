    ...
    const handleSubmit = () => {
        axios.post(collections.server_base + "/submit-otp", { email: email, otp: otp }).then((res) => {
            if (res.data.verify === true) {
                ...
                if (res.data.err === "otpErr") setOtpErr(true)
                if (res.data.err === "timeError") setTimeErr(true)
            }
        })
    }
    const handleResend = () => axios.post(collections.server_base + "/get-otp-email", { email: email }).then((res) => window.location.reload())
