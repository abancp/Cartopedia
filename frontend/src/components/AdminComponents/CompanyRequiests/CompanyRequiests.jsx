...
function CompanyRequiests() {
  ...
  useEffect(() => {
    ...
    axios.get(collections.server_base + "/admin/requests/company", { headers }).then((res) => {
  ...
  const handleAccept = (allow, email) => {
    ...
    axios.post(collections.server_base + "/admin/permission/company", { email, permission: allow }, { headers })
    setCompanyReqiuests((products) => products.filter((request) => request.email !== email))

  }

  if (companyRequiests.length > 0) {
    return (
